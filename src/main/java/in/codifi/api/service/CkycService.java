...
public class CkycService implements ICkycService {
	...
	public ResponseModel ckyc(ResponseCkyc ckyc) {
		...
		try {
			...
			RequestBody body = RequestBody.create(mediaType, "{\r\n    \"id_type\": \""+EkycConstants.PAN_TYPE+"\",\r\n    \"id_num\": \""+isUserPresent.get().getPanNumber()+"\",\r\n    \"full_name\": \""+isUserPresent.get().getUserName()+"\",\r\n    \"gender\": \""+ckyc.getGender()+"\",\r\n    \"dob\": \""+ckyc.getDob()+"\",\r\n    \"req_id\": \""+ckyc.getApplicationid()+"\"\r\n}");
			Request request = new Request.Builder().url(props.getCkycapi()).method(EkycConstants.HTTP_POST, body)
					.addHeader(EkycConstants.DIGI_CONST_TOKEN, EkycConstants.CKYC_TOKEN)
					.addHeader(EkycConstants.CONSTANT_CONTENT_TYPE,EkycConstants.CONST_APPLICATION_JSON).build();
