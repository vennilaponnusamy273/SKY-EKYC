...
//		if(validateMICRCode(bankEntity.getMicr())){
			if (user.isPresent() && user.get().getSmsVerified() > 0 && user.get().getEmailVerified() > 0
				...
				OldBanksEntity oldBanks = oldBanksRepository.findByIfsc(bankEntity.getIfsc());
				if (oldBanks == null) {
					...
					rejectionStatusHelper.insertArchiveTableRecord(bankEntity.getApplicationId(),
							EkycConstants.PAGE_BANK);
					if (updatedEntity != null && updatedEntity.getId() > 0) {
						...
						iDigioPennyDropService.createPennyDrop(bankEntity.getApplicationId());
						...
						responseModel = commonMethods
								.constructFailedMsg(MessageConstants.ERROR_WHILE_SAVING_BANK_DETAILS);
					}
				} else {
...
//		}
			...
			commonMethods
					.sendErrorMail("An error occurred while processing your request. In SaveBankDetails for the Error: "
	...
	public ResponseModel getBankByAppId(long applicationId) {
		...
		try {
			...
			commonMethods
					.sendErrorMail("An error occurred while processing your request. In getBankByAppId for the Error: "
							+ e.getMessage(), "ERR-001");
			responseModel = commonMethods.constructFailedMsg(e.getMessage());
		}
		return responseModel;
	...
	public ResponseModel getBankAdd(String ifsc) {
		...
		try {
			OldBanksEntity oldBanks = oldBanksRepository.findByIfsc(ifsc);
			if (oldBanks == null) {
			...
			if (e.getResponse().getStatus() == Response.Status.NOT_FOUND.getStatusCode()) {
				...
				commonMethods.SaveLog(null, "BankService", "getBankAdd", e.getMessage());
				commonMethods
						.sendErrorMail("An error occurred while processing your request. In getBankAdd for the Error: "
								+ e.getMessage(), "ERR-001");
	...
	public ResponseModel createPayment(PaymentEntity paymentEntity) {
		...
		try {
			...
			if (rzpayModel.getStat() == 1) {
				...
				if (order != null) {
					...
					if (savedEntity != null) {
						...
						responseModel = commonMethods
								.constructFailedMsg(MessageConstants.ERROR_WHILE_SAVE_CREATE_PAYMENT);
					}
				} else {
			...
			commonMethods
					.sendErrorMail("An error occurred while processing your request. In createPayment for the Error: "
							+ e.getMessage(), "ERR-001");
			responseModel = commonMethods.constructFailedMsg(e.getMessage());
		}
		return responseModel;
	...
	public ResponseModel verifyPayment(PaymentEntity params) {
		...
		try {
			...
			if (paymentEntity != null) {
				...
				if (isEqual) {
					...
					if (savedEntity != null) {
						...
						responseModel = commonMethods
								.constructFailedMsg(MessageConstants.ERROR_WHILE_SAVE_VERIFY_PAYMENT);
					}
				} else {
			...
			commonMethods
					.sendErrorMail("An error occurred while processing your request. In verifyPayment for the Error: "
							+ e.getMessage(), "ERR-001");
			responseModel = commonMethods.constructFailedMsg(e.getMessage());
		}
		return responseModel;
	...
	public ResponseModel checkPayment(long applicationId) {
		...
		try {
			...
			commonMethods.sendErrorMail(
					"An error occurred while processing your request. In checkPayment for the Error: " + e.getMessage(),
					"ERR-001");
			responseModel = commonMethods.constructFailedMsg(e.getMessage());
		}
		return responseModel;
	...
	public ResponseModel oldBankDetails(FormDataModel fileModel) {
		FileUpload file = fileModel.getFile();
		ResponseModel responseModel = new ResponseModel();
		List<OldBanksEntity> bankList = new ArrayList<>();
...
//			Sheet sheet = workbook.getSheetAt(0);
				for (int i = 1; i < sheet.getPhysicalNumberOfRows(); i++) {
					...
					OldBanksEntity bankDetails = new OldBanksEntity();
					bankDetails.setIfsc(row.getCell(0).getStringCellValue());
					bankDetails.setBankName(row.getCell(1).getStringCellValue());
					bankList.add(bankDetails);
					if (bankList.size() >= 1000) {
						oldBanksRepository.saveAll(bankList); // 21,404
						bankList.clear();
					}
				}
			oldBanksRepository.saveAll(bankList);
			responseModel.setMessage(EkycConstants.SUCCESS_MSG);
			responseModel.setStat(EkycConstants.SUCCESS_STATUS);
		} catch (Exception e) {
			e.printStackTrace();
