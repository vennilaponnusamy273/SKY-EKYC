...
public class DigioPennyDropService implements IDigioPennyDropService {
	...
	@Inject
	RazorpayIfscRestService razorpayIfscRestService;
	@Inject
	...
	public ResponseModel createPennyDrop(long applicationId) {
		...
		try {
			...
			if (bankCheck(savedBankEntity)==0) {
				if (savePennyEntity == null
					...
					if (savedBankEntity != null && user.isPresent()) {
						...
						if (pennyRes != null) {
							savePennyEntity = updatePennyVerificationEntity(applicationId, pennyRes, savePennyEntity,
									savedBankEntity.getAccountNo());
			...
			commonMethods
					.sendErrorMail("An error occurred while processing your request. In createPennyDrop for the Error: "
